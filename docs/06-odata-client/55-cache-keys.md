...
| 4   | Subtype cast on a collection — the FQN is `Library.Catalog.Book`, shortened here by `Library.Catalog`'s auto-synthesized alias (see [Namespace Aliasing](../generator/configuration#namespace-aliasing)); `disableAutoAlias` would keep the full name instead | `$select` on `Media/Library.Catalog.Book`                                        | `["Media", "list", { cast: "Catalog.Book" }]`                                                        |
...
| 18  | Bound operation/action — its own namespace-qualified name, the literal URL segment; shortened here the same way row 4 is, by `Library.Circulation`'s auto-synthesized alias                                                                                   | `POST /Members(42)/Library.Circulation.BulkRenew`                                | `["Members", "detail", 42, "Circulation.BulkRenew"]`                                                 |
