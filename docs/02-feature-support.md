...
| Optimistic concurrency (ETag)      | ✅  | ✅  | Automatic `If-Match`, if the service annotates the set: see [Optimistic Concurrency](./odata-client/optimistic-concurrency)      |
| Relationship management via `$ref` | ❌  | ➖  |                                                                                                                                  |
| Batch requests                     | ✅  | ✅  | `service.batch()`, multipart by default, JSON where the server supports it — see [Batch Requests](./odata-client/batch-requests) |
...
| Enum types                   | ✅  | ➖  | V2 knows no enums; as string enum, numeric enum or string union; no `IsFlags` support                      |
| Complex types                | ✅  | ✅  |                                                                                                            |
...
| Vocabulary annotations       | 🔶  | 🔶  | Basically only some `Core.*` terms                                                                         |
